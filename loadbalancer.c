...
int main(){
	...
	perror("fall to bind the socket to server");
	return 1;
	}
	...
	if(strcmp(client_message,"boy")==0){
		
		serversocket1=socket(AF_INET,SOCK_STREAM,0);
		server1.sin_family=AF_INET;
		server1.sin_addr.s_addr=inet_addr("192.168.114.129");
